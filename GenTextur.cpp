...
int main() {
    ...
    cout << "Le format est PGM (PPM gris ASCII : https://fr.wikipedia.org/wiki/Portable_pixmap)." << endl;
    cout << "Vous pouvez utiliser un logiciel de type GIMP pour les couleurs et la conversion. (https://www.gimp.org/)" << endl;
    // Par défaut :
    const int MAXBlancMAX = 65536; // Norme du fichier.
    const int FULLWHITE = 256; // < MAXBlancMAX( Norme du fichier ).

    int X=16; // Colonnes
    int Y=16; // Lignes // 70 caractères MAX par ligne.
    bool GTend=true; // Ne pas recommencer.
    do {
        // Début du fichier :
        int PixFin=0;
        string const nomFichier("GenTex.ppm"); // input ?
        ofstream myfile;
